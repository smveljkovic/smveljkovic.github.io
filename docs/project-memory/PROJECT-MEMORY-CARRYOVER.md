...
> 
> The Astro migration is now in Stage 3:
> 
> ```text
...
> 
> Distilled assistant rules now live in:
> 
> ```text
> .aiassistant/rules/
> ```
...
> 
> - Distinguish Stevan as `Person`, the reviewed work, the published
> review/article, and the local hosted manuscript/page.
...
> - Current local review type `["ScholarlyArticle", "Review"]` is acceptable;
> Stage 3 may add or finalize a `WebPage` node pattern.
...
> ```
> 
> Publications schema factory path to verify/use:
> 
> ```text
...
> 
> ## 7. Decisions and constraints to preserve
> 
...
> ```
> 
> Use an `absoluteUrl(path)` helper via `new URL(path, site.url).toString()` to
...
> 
> Christian Right special rule: reviewed book is an edited volume, so use `editor:
> Gionathan Lo Mascolo`, not author with “(ed.)”.
> 
...
> non-draft pages.
> 4. Confirm review pages pass SEO props through:
> 
> ```text
> review page → ReviewLayout → BaseLayout → SeoHead
> ```
> 
> 5. Confirm review pages use intended Open Graph article metadata.
> 6. Harden JSON-LD node IDs so no `@id` can become an array.
> 7. Normalize DOI handling consistently, including `doi.org` and `dx.doi.org`
> variants.
> 8. Confirm `/publications/` is generated from both `reviews` and
> `publicationItems`.
> 9. Compare rendered `/publications/` against the intended bibliography,
> including year grouping, sort dates, manual order, citation formatting, DOI
> links, local links, PDF links, and note lines.
> 10. Harden `/publications/` `CollectionPage` / `ItemList` JSON-LD; item IDs
> should prefer DOI URL, then canonical local page URL, then stable local fragment
> URL.
> 11. Refine review JSON-LD: possible `WebPage` node pattern, `name` vs
> `headline`, periodical/volume/issue chain, article IDs, pagination, DOI, ISSNs,
> date semantics, version wording.
> 12. Verify Christian Right metadata: exact published date, volume/issue,
> `csaf039`, ISSNs, PDF filename/path, version wording.
> 13. Add the three new review pages as drafts:
> 
> ```text
> src/content/reviews/godless-crusade.md
> src/content/reviews/hell-christian-ecology.md
...
> ```
> 
> 14. Define consistent copyright, version, and review-page display rules before
> publishing additional local review texts.
> 15. Create `docs/legacy-urls.md`; inventory old HTML pages, PDFs, CV paths,
> image paths, and externally linked assets.
> 16. Add static redirect stubs for GitHub Pages where needed; keep old PDF URLs
> serving PDFs where feasible.
> 17. Decide thesis-page scope for Stage 3.
> 
...
> 
> 18. Thesis page planned route if implemented:
> 
> ```text
> src/pages/thesis/religious-atavism-climate-crisis/index.astro
> ```
> 
> 19. Create/update deployment documentation and launch checklist.
> 20. Confirm GitHub Pages workflow, custom domain, DNS, and `public/CNAME` needs.
> 21. Fix or consciously defer small accessibility/design issues, including Safari
> homepage `h1.Name` visibility.
> 22. Seminars consolidation remains undecided.
> 
...
> ```
> 
> - Homepage uses `homeSchema`.
> - CV uses `cvSchema`.
> - No active route imports `buildReviewSchema`.
> - Old Cosmic schema remains reference only.
> 
> 5. Harden current JSON-LD and SEO output before adding live pages.
